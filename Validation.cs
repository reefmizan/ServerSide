...
namespace WpfClientReef
{
    ...
    public class ValidDesName : ValidationRule
    {
        public int Min { get; set; }
        public int Max { get; set; }
        public override ValidationResult Validate(object value, CultureInfo cultureInfo)
        {
            try
            {
                ...
                for (int i = 0; i < name.Length; i++)
                {
                    if (!Char.IsLetterOrDigit(name[i]) && !Char.IsWhiteSpace(name[i]) && !Char.IsPunctuation(name[i]))
                    {
                        return new ValidationResult(false, "must be a letter, digit, space or Punctuation");
                    }
                }
